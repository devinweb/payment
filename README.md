...
Once all configuration steps are done, you are ready to use this package in your app:
before start this package need also to setup the front end, you can use The Form in the `exemple` section to check how you can setup it.
...
To pay via payfort it's simple, but before process the payment it's requiered to setup your `FrontEnd`, you can check the [`payment-boilerplate`](https://github.com/devinweb/payment-boilerplate) repos

## Configuration

Before start using Payfort as a payment gateway, you will add the credentiels to `app/config/payments/php`
...
Now you are ready to use Payfort, you need three Routes: one for redirecting if payfort response with error, the second if the payment is successfuly and the last one used to send data from front end to the backend
...
to use payment via apple the the same as before
